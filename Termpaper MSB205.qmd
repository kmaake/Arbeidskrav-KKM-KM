...
```{r modell 1}
mod1 <- "price ~ bedrooms + bathrooms + sqft_living + sqft_lot + floors + waterfront + view + condition + grade + yr_built + yr_renovated + sqft_living15 + sqft_lot15 + sqft_above + sqft_basement + year_month"
```
...
```{r modell 2}
mod2 <- "price ~ bedrooms + bathrooms + sqft_living + sqft_lot + floors + waterfront + view + condition + yr_built + yr_renovated + sqft_living15 + sqft_lot15 + sqft_above + sqft_basement + grade + year_month + dist_cbd_km + linguist_2 + poverty_pe + POC_percen + unemploy_2 + sen_pop_pe + facilities + wastewater + traffic_pe + diesel + transporta + housing_pe + ozone + PM25 + toxic_rele + superfund + hazardous_ + lead_perce + socio_perc"
```
...
```{r modell 3}
mod3 <- "price ~ bedrooms + bathrooms + sqft_living + sqft_living15 + sqft_lot + floors + waterfront + view + condition + yr_built + yr_renovated + sqft_above + sqft_basement + grade + dist_cbd_km + EHD_percen + low + high + year_month"
```
